...
fasterUniFrac <- function(o.t, tree, distance="UniFrac_unweighted", blocksize=1000, use.cores) {
  ...
  tip.ages = node.depth.edgelength(tree);
  #Keep only tips
...
fast.cophenetic.phylo <- function(x, nblocks=1000, use.cores=detectCores()) {
  ...
  my.node_paths <- mclapply(seq(1, Ntip(x)), function (tip) {nodepath(x, from=tip, to=phylobase::getNode(as(x, "phylo4"), phylobase::nTips(as(x, "phylo4")) + 1))}, mc.cores=use.cores);
  print(my.node_paths)
